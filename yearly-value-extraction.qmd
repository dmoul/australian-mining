...
Trend for each commodity sorted from greatest to least peak value (units vary).
...
dta_yearly_long |>
  ...
  labs(
    ...
    subtitle = glue("Highest to lowest peak values. {year_first}-{year_last}"),
    x = NULL,
...
```{r}
#| label: fig-value-yearly-grouped-by-scale
#| fig-cap: "Yearly Australian mining value extracted"
#| eval: false

walk(n_groups:1, plot_group_value)
walk(n_groups:1, plot_group_value_2021)
...
dta_yearly_long |>
  ...
  fmt_currency(columns = c(value_2021, value_max_2021),
             ...
             currency = "AUD",
             suffixing = TRUE) |>
  fmt_percent(columns = value_pct_of_max,
             decimals = 0) |>
  sub_missing() |>
